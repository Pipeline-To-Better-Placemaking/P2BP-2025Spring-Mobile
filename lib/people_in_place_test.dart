...
class PeopleInPlaceTest extends StatefulWidget {
  final List<LatLng> polygonPoints;
  final Set<Polygon> polygon;
  const PeopleInPlaceTest({
    Key? key,
  ...
  @override
  State<PeopleInPlaceTest> createState() => _PeopleInPlaceTestState();
}

class _PeopleInPlaceTestState extends State<PeopleInPlaceTest> {
  ...
  // Custom marker icons
  BitmapDescriptor? maleMarker;
  BitmapDescriptor? femaleMarker;
  BitmapDescriptor? naMarker;
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  // Optionally, you can remove the checkbox if you don't need it for now.
                  // Here it remains for future use.
                  Checkbox(
                    value: false,
                    onChanged: (_) {},
                  ),
  ...
  // Pop Up Menu with Shared Preferences
  // void _showInstructionOverlayIfNeeded() async {
  //   final prefs = await SharedPreferences.getInstance();
  //   bool shouldShow = prefs.getBool('showInstructions') ?? true;
  //   if (shouldShow) {
  ...
  //             mainAxisSize: MainAxisSize.min,
  //             children: [
  //               Text('Tap on the map to log a data point.'),
  //               Row(
  //                 children: [
  ...
  //               onPressed: () {
  //                 if (dontShowAgain) {
  //                   prefs.setBool('showInstructions', false);
  //                 }
  //                 Navigator.of(context).pop();
  //               },
  //               child: Text('OK'),
  ...
  // New tap handler for People In Place
      ...
      builder: (BuildContext bottomSheetContext) {
        return PeopleInPlaceClassificationSheet(
          onSubmit: (classificationData) {
            ...
            switch (classificationData['gender']) {
              case 'Male':
                markerIcon = maleMarker ?? BitmapDescriptor.defaultMarker;
                break;
              case 'Female':
                markerIcon = femaleMarker ?? BitmapDescriptor.defaultMarker;
                break;
              default:
                markerIcon = naMarker ?? BitmapDescriptor.defaultMarker;
            }
  ...
  Future<void> _loadCustomMarkers() async {
    ...
    try {
      maleMarker = await AssetMapBitmap.create(
        configuration,
        'assets/custom_icons/male_marker.png',
        width: 36,
        height: 36,
      );
      femaleMarker = await AssetMapBitmap.create(
        configuration,
        'assets/custom_icons/female_marker.png',
        width: 36,
        height: 36,
      );
      naMarker = await AssetMapBitmap.create(
        configuration,
        'assets/custom_icons/na_marker.png',
        width: 36,
        height: 36,
      );
      setState(() {
